...
import type { FileChange, Hunk, StackCommit } from '../../common/types'
...
export const getCommitHistory = async (
  ...
  try {
    ...
    logger.info(
      `Branch comparison - custom current: ${customCurrentBranch}, custom base: ${customBaseBranch}`
    )
    logger.info(
      `Branch comparison - resolved current: ${currentBranch}, resolved base: ${baseBranch}`
    )
